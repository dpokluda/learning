...
| 6/9/2026 | Step 7   | CNN: Conv(1→32,3,pad1)→ReLU→Pool → Conv(32→64,3,pad1)→ReLU→Pool → Flatten(64·7·7=3136) → Linear(3136,128)→ReLU→Linear(128,10). See step7/results.md. (1) SGD lr=0.01 5ep → 96.81% — *below* the MLP, but undertrained (loss still free-falling, no plateau), not a worse architecture. (2) Swap to Adam lr=1e-3 → **99.10%** in 5 epochs (target hit). (3) Adam 10 epochs: train loss 0.027→0.006 (4.5× lower) but test acc unchanged at 99.10% — textbook overfitting/diminishing returns: more training just memorizes harder, no generalization gain. ~99% is the ceiling for this approach. Arc: 92% linear → 98% MLP → 99% CNN, each jump from a real architectural change. Gotcha: removed manual flatten from BOTH train() and evaluate() (Conv needs 4-D [N,1,28,28]); nn.Flatten() inside model handles it after conv. |
